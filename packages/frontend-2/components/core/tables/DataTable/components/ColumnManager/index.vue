...
// Local column state
const localColumns = ref<TableColumnDef[]>([...props.columns])
const localDetailColumns = ref<TableColumnDef[]>([...(props.detailColumns || [])])
...
function handleAdd(column: TableColumnDef): void {
  const target = currentView.value === 'parent' ? localColumns : localDetailColumns
  target.value.push({ ...column })
  listRefreshKey.value++
}

function handleRemove(column: TableColumnDef): void {
  const target = currentView.value === 'parent' ? localColumns : localDetailColumns
...
function handleVisibilityChange(column: TableColumnDef, visible: boolean): void {
  const target = currentView.value === 'parent' ? localColumns : localDetailColumns
  const index = target.value.findIndex((col) => col.id === column.id)
  if (index !== -1) {
...
function handleDragStart(event: DragEvent, column: TableColumnDef): void {
