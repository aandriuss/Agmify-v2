...
const emit = defineEmits<{
  ...
  'column-resize': [{ element: HTMLElement }]
...
function handleExpandedRowsUpdate(value: unknown): void {
  try {
    ...
    emit('error', { error: new TableError('Failed to update expanded rows', error) })
  }
}
...
function handleColumnResize(event: { element: HTMLElement }): void {
  try {
    ...
    emit('error', { error: new TableError('Failed to handle column resize', error) })
  }
}
...
function handleColumnReorder(event: { dragIndex: number; dropIndex: number }): void {
  try {
    ...
    emit('error', { error: new TableError('Failed to handle column reorder', error) })
  }
}
...
function handleSort(event: DataTableSortEvent): void {
  try {
    ...
    emit('error', { error: new TableError('Failed to handle sort', error) })
  }
}
...
function handleFilter(event: DataTableFilterEvent): void {
  try {
    ...
    emit('error', { error: new TableError('Failed to handle filter', error) })
  }
}
...
function handleRowExpand(event: { data: BaseTableRow }): void {
  try {
    ...
    emit('error', { error: new TableError('Failed to handle row expand', error) })
  }
}
...
function handleRowCollapse(event: { data: BaseTableRow }): void {
  try {
    ...
    emit('error', { error: new TableError('Failed to handle row collapse', error) })
  }
}
