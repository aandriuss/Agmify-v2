...
const localDetailColumns = ref<ColumnDef[]>([])
const tempColumns = ref<ColumnDef[]>([])
const tempDetailColumns = ref<ColumnDef[]>([])
...
const expandedRows = computed<BaseTableRow[]>({
  ...
  set: (value) => {
    if (Array.isArray(value)) {
      ...
      emit('update:expandedRows', { rows: validRows })
    }
  }
})
...
function handleExpandedRowsUpdate(event: { rows: BaseTableRow[] }): void {
  if (Array.isArray(event.rows)) {
    const validRows = validateTableRows(event.rows)
    expandedRowsState.value = validRows
    emit('update:expandedRows', { rows: validRows })
  }
}

function handleRowExpand(event: { row: BaseTableRow }): void {
  emit('row-expand', { row: event.row })
}

function handleRowCollapse(event: { row: BaseTableRow }): void {
  emit('row-collapse', { row: event.row })
}
...
function handleColumnReorder(event: ColumnReorderEvent): void {
  try {
    isSaving.value = true
    ...
    emit('update:columns', { columns: updatedColumns })
...
function handleSort(event: { field: string; order: number }): void {
