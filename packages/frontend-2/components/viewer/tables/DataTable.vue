                ...
                >
                  ...
                  <div class="space-y-1 pl-2">
                    <template v-for="param in group.parameters" :key="param.field">
                      <ParameterItem
                        :parameter="columnDefToParameter(param)"
          ...
          >
            ...
            <div class="flex-1 overflow-y-auto p-1 space-y-1">
              <button
                ...
                :key="column.field"
                type="button"
                class="flex items-center justify-between p-2 hover:bg-gray-50 rounded text-sm w-full"
                draggable="true"
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <ParameterBadge :parameter="columnDefToParameter(column)" />
                </div>
                ...
                <div class="flex items-center gap-2">
                  ...
                  <Checkbox
                    ...
                    :binary="true"
                    @change.stop="handleVisibilityChange(column)"
                  />
                </div>
              </button>
            </div>
...
import { createColumnDef, columnDefToParameter } from '~/composables/parameters/'
...
const emit = defineEmits<CombinedTableEvents>()
...
const filteredParameters = computed(() => {
  let result = currentAvailableParameters.value
...
if (props.columns) {
  localParentColumns.value = props.columns.map((col) => createColumnDef(col))
  tempParentColumns.value = props.columns.map((col) => createColumnDef(col))
}

if (props.detailColumns) {
  localChildColumns.value = props.detailColumns.map((col) => createColumnDef(col))
  tempChildColumns.value = props.detailColumns.map((col) => createColumnDef(col))
}

// Column management
const openDialog = () => {
  tempParentColumns.value = localParentColumns.value.map((col) => createColumnDef(col))
  tempChildColumns.value = localChildColumns.value.map((col) => createColumnDef(col))
  dialogOpen.value = true
}

const applyChanges = () => {
  // Update local state
  localParentColumns.value = tempParentColumns.value.map((col) => createColumnDef(col))
  localChildColumns.value = tempChildColumns.value.map((col) => createColumnDef(col))
...
const handleDropToActive = (_event: DragEvent) => {
...
const onColumnResize = (event: ColumnResizeEvent) => {
  const { element } = event
...
const handleVisibilityChange = (column: ColumnDef) => {
  ...
  if (targetColumn) {
    targetColumn.visible = column.visible
  }
}

const handleColumnReorder = (event: DataTableColumnReorderEvent) => {
  ...
  // Update order after move
  const reorderedColumns = columns.value.map((col, index) =>
    createColumnDef({
      ...col,
      order: index
    })
